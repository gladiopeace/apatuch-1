...
await jimp.read(lea.toBuffer())
.then(lenna => {
return lenna
.resize(282, 150) // resize
.quality(100) // set JPEG quality
// .greyscale() // set greyscale
.write(`tmp/lea.jpg`) // save
})
...
conn.sendLoc(jid, action === 'add' ? h1 : ilea, text, global.footer, 'LIST MENU', '#menu', false, {
                contextInfo: {
                  mentionedJid: [user]
                }
              })
...
//let iwel = fs.readFileSync('tmp/hwel.jpg')
//let ilea = fs.readFileSync('tmp/lea.jpg')


/*await conn.sendLoc(jid, action === 'add' ? wbuf : ilea, text, global.footer, 'LIST MENU', '#menu', false, {
                contextInfo: {
                  mentionedJid: [user]
                }
              }) */
